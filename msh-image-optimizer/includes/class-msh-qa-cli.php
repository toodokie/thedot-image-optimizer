...
if (defined('WP_CLI') && WP_CLI) {
    ...
    class MSH_QA_CLI_Command {
        public function __invoke($args, $assoc_args) {
            $rename_ids    = isset($assoc_args['rename']) ? array_filter(array_map('intval', explode(',', $assoc_args['rename']))) : [];
            $rename_mode   = isset($assoc_args['rename-mode']) ? strtolower($assoc_args['rename-mode']) : 'test';
            $optimize_ids  = isset($assoc_args['optimize']) ? array_filter(array_map('intval', explode(',', $assoc_args['optimize']))) : [];
            ...
            if ($run_duplicate) {
                ...
                if (is_wp_error($report)) {
                    ...
                    if ($groups_found === 0) {
                        ...
                        $table_rows = array_map(function($group) {
                        ...
                        if (!empty($table_rows)) {
                            ...
                            WP_CLI\Utils\format_items('table', $table_rows, ['group', 'files', 'published', 'cleanup_ready', 'keeper_id']);
                        }
                    }
                }
            }
