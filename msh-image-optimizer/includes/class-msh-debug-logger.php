...
class MSH_Debug_Logger {
     ...
     */
    public function log($message, $context = 'GENERAL', $data = []) {
        if (!$this->enabled) {
            return;
        }
    ...
    public function log_resolver($attachment_id, $expected_path, $found_path, $method, $mismatch) {
        ...
        if ($mismatch) {
            $this->log(
                sprintf('MISMATCH RESOLVED: Attachment %d - Expected "%s" → Found "%s"',
                    $attachment_id,
     ...
     */
    public function log_analyzer($attachment_id, $status, $details = []) {
        if (!$this->enabled) {
            return;
        }
     ...
     */
    public function log_verification($operation_id, $attachment_id, $result, $details = []) {
        if (!$this->enabled) {
            return;
        }

        $this->log(
            sprintf('Verification [%s] - Attachment %d - Result: %s',
     ...
     */
    public function log_rename($attachment_id, $old_filename, $new_filename, $status, $details = []) {
        if (!$this->enabled) {
            return;
        }

        $this->log(
            sprintf('Rename: Attachment %d - "%s" → "%s" [%s]',
                $attachment_id,
                $old_filename,
                $new_filename,
                $status
     ...
     * Log error
     */
    public function log_error($message, $context = 'ERROR', $details = []) {
        if (!$this->enabled) {
            return;
        }
     ...
     * Log warning
     */
    public function log_warning($message, $context = 'WARNING', $details = []) {
        if (!$this->enabled) {
            return;
        }
     ...
     * Log success
     */
    public function log_success($message, $context = 'SUCCESS', $details = []) {
        if (!$this->enabled) {
            return;
        }
