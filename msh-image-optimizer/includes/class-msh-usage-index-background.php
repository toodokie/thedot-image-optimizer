...
class MSH_Usage_Index_Background {
    ...
    private function __construct() {
        ...
        add_action('add_attachment', [$this, 'handle_attachment_change'], 20);
        add_action('edit_attachment', [$this, 'handle_attachment_change'], 20);
        add_action('delete_attachment', [$this, 'handle_attachment_deletion'], 20);

        add_action('wp_ajax_msh_queue_usage_index_rebuild', [$this, 'ajax_queue_rebuild']);
        add_action('wp_ajax_msh_get_usage_index_status', [$this, 'ajax_get_status']);
    }
    ...
    public function handle_attachment_change($attachment_id) {
        ...
        $this->queue_attachments([$attachment_id], 'attachment_change');
    }
    ...
    public function handle_attachment_deletion($attachment_id) {
        ...
        if (method_exists($usage_index, 'cleanup_orphaned_entries')) {
            $usage_index->cleanup_orphaned_entries([$attachment_id]);
        }
    }
    ...
    public function process_queue() {
        ...
        try {
            ...
            $pending_queue = count($state['queue'] ?? []);
    ...
    public function get_status_for_ui($include_summary = true) {
        ...
        $status = [
            ...
            'errors'            => array_slice((array) ($state['errors'] ?? []), -5),
            'messages'          => array_slice((array) ($state['messages'] ?? []), -5),
        ];
