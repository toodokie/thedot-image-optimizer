...
class MSH_Metadata_Regeneration_Background {
    ...
    private function __construct() {
        ...
        // AJAX endpoints
        add_action('wp_ajax_msh_start_ai_regeneration', [$this, 'ajax_start_regeneration']);
        add_action('wp_ajax_msh_pause_ai_regeneration', [$this, 'ajax_pause_regeneration']);
        add_action('wp_ajax_msh_resume_ai_regeneration', [$this, 'ajax_resume_regeneration']);
        add_action('wp_ajax_msh_cancel_ai_regeneration', [$this, 'ajax_cancel_regeneration']);
        add_action('wp_ajax_msh_get_ai_regeneration_status', [$this, 'ajax_get_status']);
    }
     ...
     */
    public function queue_regeneration($attachment_ids, $options = []) {
        ...
        if (!empty($current_state) && in_array($current_state['status'], ['queued', 'running'], true)) {
        ...
        $fields = isset($options['fields']) && is_array($options['fields'])
            ? $options['fields']
            : ['title', 'alt_text', 'caption', 'description', 'filename'];
    ...
    public function process_queue() {
        ...
        try {
            ...
            if ($state['status'] === 'queued') {
                ...
                $this->save_state($state);
                $this->update_job_status($state['job_id'], 'running', ['started_at' => time()]);
            }
            ...
            if (empty($state['queue'])) {
                ...
                $this->update_job_status($state['job_id'], 'completed', [
                    'completed_at' => time(),
                ...
                // Update job progress
                $this->update_job_status($state['job_id'], $state['status'], [
                    'processed' => $state['processed'],
                    'succeeded' => $state['succeeded'],
                    'failed' => $state['failed'],
                    'skipped' => $state['skipped'],
    ...
    private function process_single_attachment($attachment_id, $state) {
        try {
            ...
            if ($state['mode'] === 'fill-empty') {
                ...
                if ($has_content) {
                    return ['success' => false, 'skipped' => true, 'message' => 'Already has metadata'];
                }
            }
            ...
            if (empty($metadata)) {
                return ['success' => false, 'skipped' => false, 'message' => 'No metadata generated'];
            }
    ...
    public function ajax_pause_regeneration() {
        ...
        $state = $this->get_state();
        if (empty($state)) {
            wp_send_json_error(['message' => 'No active job']);
        }
    ...
    public function ajax_resume_regeneration() {
        ...
        if (empty($state) || $state['status'] !== 'paused') {
            wp_send_json_error(['message' => 'No paused job to resume']);
        }
    ...
    public function ajax_cancel_regeneration() {
        ...
        $state['completed_at'] = time();
        $this->save_state($state);
        $this->update_job_status($state['job_id'], 'cancelled', ['completed_at' => time()]);
    ...
    private function get_jobs() {
        return get_option(self::JOBS_OPTION, []);
    }
    ...
    private function update_job_status($job_id, $status, $updates = []) {
        $jobs = $this->get_jobs();
