# Phase 1F: Complete DELETE/ADD Guide
...
### A4: Add Batch AI Mode Getter to MSH_Image_Optimizer Class
...
**ADD THESE LINES AFTER** the `define( 'MSH_IN_OPTIMIZE_BATCH', true );`:
```php
	if ( ! defined( 'MSH_IN_OPTIMIZE_BATCH' ) ) {
		define( 'MSH_IN_OPTIMIZE_BATCH', true );
	}
	...
	// ... rest of batch processing continues unchanged ...
```
...
### Files Modified
- **includes/class-msh-image-optimizer.php** - 6 locations
- **msh-image-optimizer.php** - 2 lines (version bump)

### What Gets Cached at Batch Start
1. **AI Mode** - `get_option('msh_ai_mode')` → cached in `$this->msh_batch_ai_mode`
2. **Context Data** - All context profiles → cached in `$this->msh_cached_context`
3. **Context Signature** - MD5 hash → cached in `$this->msh_cached_signature`
...
**END OF PHASE 1F DELETE/ADD GUIDE**
