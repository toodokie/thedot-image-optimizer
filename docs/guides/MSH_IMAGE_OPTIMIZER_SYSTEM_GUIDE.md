...
2. **Smart Metadata & Filenames** – Deterministic templates tuned for healthcare/local SEO, optional AI overlay, and controlled filename suggestions.
   📘 **See:** [METADATA_RULEBOOK.md](../architecture/METADATA_RULEBOOK.md) – Canonical source of truth for AI and Non-AI metadata generation rules
...
4. **Reset safely** – Reset buttons/CLI clear every staging key (`_msh_ai_staged_meta`, `_msh_ai_keywords`, `_msh_context_trace`, `_msh_confidence*`, `_msh_suggested_filename`, `_msh_suggested_filename_context`, `_msh_ai_filename_slug`, `msh_filename_last_suggested`) plus the analyzer transient so the next Analyze pass re-stages metadata and (when rename suggestions are enabled) filename proposals for both AI + deterministic flows. Core WordPress fields (`_wp_attached_file`, `_wp_attachment_metadata`, titles/captions/ALT, GUID/URLs) remain untouched and nothing on disk moves until Optimize.  
