...
class MSH_Perceptual_Hash {
	 ...
	 * @var int[]
	 */
	private static $bitCountMap = array();
	 ...
	 * @return array
	 */
	public function group_similar( array $records, array $options = array() ) {
		$thresholds   = $this->resolve_thresholds( $options['thresholds'] ?? array() );
	...
	private function normalize_palette_histograms( $hist ) {
		...
		$channels   = array( 'r', 'g', 'b', 'l' );
		$normalized = array();

		foreach ( $channels as $channel ) {
			$values                 = isset( $hist[ $channel ] ) && is_array( $hist[ $channel ] ) ? $hist[ $channel ] : array();
	...
	public function get_palette_signature( $attachment_id ) {
		...
		if ( $normalized_stored ) {
			if ( ! is_array( $normalized_stored['hist'] ) || empty(
				array_filter(
					$normalized_stored['hist'],
