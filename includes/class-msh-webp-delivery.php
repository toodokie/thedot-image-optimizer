...
class MSH_WebP_Delivery {
	...
	public function ajax_detect_webp() {
		...
		check_ajax_referer( 'msh-webp-detect', 'nonce' );

		// Sanitize input
		$supported = isset( $_POST['webp_supported'] ) && rest_sanitize_boolean( wp_unslash( $_POST['webp_supported'] ) );

		// Set cookie for future requests
		setcookie( 'webp_support', $supported ? '1' : '0', time() + YEAR_IN_SECONDS, '/' );

		wp_send_json_success( array( 'webp_supported' => $supported ) );
	}
	...
	public function get_webp_status() {
		return array(
			...
			'cookie_value'     => isset( $_COOKIE['webp_support'] ) ? sanitize_text_field( wp_unslash( $_COOKIE['webp_support'] ) ) : 'not set',
			'user_agent'       => isset( $_SERVER['HTTP_USER_AGENT'] ) ? sanitize_text_field( $_SERVER['HTTP_USER_AGENT'] ) : 'not available',
			'accept_header'    => isset( $_SERVER['HTTP_ACCEPT'] ) ? sanitize_text_field( $_SERVER['HTTP_ACCEPT'] ) : 'not available',
		);
	}
}
