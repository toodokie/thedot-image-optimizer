...
class MSH_Content_Usage_Lookup {
	...
	private function __construct() {
		...
		add_action( $this->scheduled_hook, array( $this, 'handle_scheduled_refresh' ) );
		add_action( 'wp_ajax_msh_trigger_incremental_refresh', array( $this, 'ajax_trigger_incremental_refresh' ) );
	}
	...
	public function register_hooks() {
		...
		add_action( 'save_post', array( $this, 'handle_post_change' ), 20, 1 );
		add_action( 'deleted_post', array( $this, 'handle_post_change' ), 20, 1 );
		add_action( 'added_post_meta', array( $this, 'handle_meta_change' ), 20, 3 );
		add_action( 'updated_post_meta', array( $this, 'handle_meta_change' ), 20, 3 );
		add_action( 'deleted_post_meta', array( $this, 'handle_meta_change' ), 20, 3 );
		add_action( 'updated_option', array( $this, 'handle_option_change' ), 20, 3 );
	}
	...
	public function get_lookup( $force = false ) {
		...
		return $this->build_lookup( true, array( 'trigger' => 'cache_miss' ) );
	}

	public function build_lookup( $force = false, array $context = array() ) {
		if ( ! $force ) {
			$cached = get_transient( $this->cache_key );
			if ( is_array( $cached ) ) {
				return $cached;
		...
		// Prepare LIKE pattern for uploads directory
		$uploads_like = '%' . $wpdb->esc_like( '/uploads/' ) . '%';
	...
	public function handle_scheduled_refresh() {
		...
		MSH_Image_Usage_Index::get_instance()->build_optimized_complete_index(
			true,
			$lookup,
			array(
				'trigger' => 'scheduler',
	...
	public function ajax_trigger_incremental_refresh() {
		...
		try {
			...
			$snapshot     = get_option( $this->snapshot_option, array() );
