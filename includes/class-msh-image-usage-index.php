...
class MSH_Image_Usage_Index {
	...
	public function get_modified_content_since_last_index() {
		...
		// Prepare LIKE pattern for custom page post types
		$page_like = '%' . $wpdb->esc_like( '_page' );
		...
		$modified_posts = $wpdb->get_col(
            ...
            AND (p.post_type IN ('post', 'page') OR p.post_type LIKE %s)
        ",
				$last_index_update,
				$page_like
			)
	...
	$image_mime_like = $wpdb->esc_like( 'image/' ) . '%';
	// phpcs:disable WordPress.DB.PreparedSQLPlaceholders.LikeWildcardsInQuery -- Column-to-column LIKE required: searching for posts containing attachment filenames. No user input involved. %% becomes % in prepared statement.
