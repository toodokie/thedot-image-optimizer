...
class MSH_Media_Cleanup {
	...
	private function get_images_for_scanning( $limit = null ) {
		...
		return $wpdb->get_results( $sql ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- SQL is prepared above, LIMIT added safely with intval()
	}
	...
	private function get_visual_scan_attachment_ids() {
		...
		$ids = $wpdb->get_col(
			$wpdb->prepare(
				"SELECT ID FROM {$wpdb->posts} WHERE post_type = %s AND post_mime_type LIKE %s AND post_status != %s ORDER BY ID ASC",
				'attachment',
				$image_mime_like,
				'trash'
			)
